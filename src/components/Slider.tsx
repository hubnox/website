...
const EventSlider: React.FC = () => {
  ...
  const filteredEvents = events
    .filter((event: Event) => visibleCreators.has(event.creator?.objectId))
    .filter((event: Event) => new Date(event.startDateAndTime.iso) >= currentDate)
    .sort(
      (a: Event, b: Event) =>
        new Date(a.startDateAndTime.iso).getTime() -
        new Date(b.startDateAndTime.iso).getTime()
    );
  ...
  return (
    <section className="section-padding-lg" id="events">
      <div className="w-full max-w-[1266px] mx-auto px-[25px] ">
        ...
        >
          {finalEvents.map((event: Event) => (
            <SwiperSlide key={event.objectId}>
              <EventSlide
                name={event.name}
                description={event.description}
                thumbnail={event.thumbnail}
                startDate={event.startDateAndTime.iso}
